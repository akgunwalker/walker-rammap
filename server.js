...
'@
$targets=ConvertFrom-Json '${targets}';$whitelist=ConvertFrom-Json '${whitelist}'
$before=(Get-CimInstance Win32_OperatingSystem).FreePhysicalMemory;$ok=0;$names=@()
Get-Process|Where-Object{$targets -contains $_.ProcessName -and $whitelist -notcontains $_.ProcessName}|ForEach-Object{try{if([MemTrim]::EmptyWorkingSet($_.Handle)){$ok++;$names+=$_.ProcessName}}catch{}}
...
function analyze(data){
  ...
  for(const p of data.processes||[]){
    ...
    if(value>500*1024*1024&&!settings.whitelist.some(x=>x.toLowerCase()===key)&&!settings.targets.some(x=>x.toLowerCase()===key))suggestions.push({name:p.Name,memory:value});
  }
